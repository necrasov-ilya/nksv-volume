...
export function resolveItemKind(entry: MetaEntry): ItemKind {
...
export function resolveItemLabel(entry: MetaEntry): string {
...
export function resolveItemIcon(entry: MetaEntry): string {
...
export function resolveViewer(entry: MetaEntry): ViewerKind {
  switch (resolveItemKind(entry)) {
...
export function entryDisplayName(entry: MetaEntry): string {
  if (entry.type === 'folder') return entry.name;
  if (entry.type === 'article') return entry.title;
  return entry.originalName;
}

export function sortKey(entry: MetaEntry): string {
  return entryDisplayName(entry).toLocaleLowerCase('ru');
}